...
export default function Home() {
    ...
    const fetchPage = React.useCallback(async () => {
        ...
        setNotFound(false);
        try {
            const res = await fetch('/api/articles/by-path/%2F', { cache: 'no-store' });
            if (res.status === 404) {
                setNotFound(true);
                setPage(null);
            } else if (!res.ok) {
                const body = await res.text();
                setError(`Failed to load page: ${res.status} ${body}`);
            } else {
    ...
    // 加载评论
    const fetchComments = React.useCallback(async (pageId: number) => {
        setCommentsLoading(true);
        try {
            const res = await fetch(`/api/articles/${pageId}/comments`);
            if (res.ok) {
                const data = await res.json();
    ...
    // 加载标签
    const fetchTags = React.useCallback(async (pageId: number) => {
        setTagsLoading(true);
        try {
            const res = await fetch(`/api/articles/${pageId}/tags`);
            if (res.ok) {
                const data = await res.json();
                setTags(data.tags || []);
            }
    ...
    // 加载作者信息
    const fetchAuthor = React.useCallback(async (authorId: number) => {
    ...
    // 当页面加载成功后，获取评论、标签和作者信息
    React.useEffect(() => {
        if (page && page.id) {
            fetchComments(page.id);
            fetchTags(page.id);
            fetchAuthor(page.authorId);
        }
    }, [page, fetchComments, fetchTags, fetchAuthor]);
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-base-100 p-4">
                <div className="flex flex-col items-center">
                    ...
                    <div className="text-lg text-base-content">正在加载首页…</div>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-base-100 p-4">
                <div className="flex flex-col items-center justify-center max-w-md text-center">
                    ...
                    <h1 className="text-3xl font-bold text-base-content mb-4">BlueWiki</h1>
                    <p className="text-base-content/70 mb-8">当前尚未创建主页数据。</p>
                    <Link 
                        href="/first-use" 
                        className="px-6 py-3 bg-primary text-primary-content rounded-full font-medium hover:bg-primary/90 transition-colors duration-200 shadow-sm"
                    >
                        开始初始设置
                    </Link>
                </div>
            ...
            <div className="max-w-4xl mx-auto px-4 py-8">
                {page ? (
                    ...
                    <div className="text-center py-12">
                        ...
                        <p className="text-base-content/70">页面加载失败，请稍后重试。</p>
                    </div>
                )}
            </div>
        </div>
